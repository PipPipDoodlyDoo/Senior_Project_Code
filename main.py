...
import Terminal_Print                                   # This will handle all the print
...
utime.sleep(Library.MAIN_LIB["READ_TIME"])

#######################################################################################
# CALIBRATION PROCESS
#######################################################################################
# INDICATE TO THE USER TO BEING CALIBRATION
Terminal_Print.clear_screen()

##################################################################################
# Calibration Routine
##################################################################################
# PRINT THE LAYOUT OF THE TERMINAL

Terminal_Print.calibration_layout()
...
while cal_prog == 0:                                                # Keep capturing the zero value until user stops with button press
    ...
    print("\u001b[H")
    print("\u001b[4B","\u001b[21C",'{:.2f}'.format(ph_cal))
    print("\u001b[H")
    print("\u001b[6B","\u001b[24C",'{:.2f}'.format(mag_cal))
